...
import {
  getActiveAccessPassForProfile,
  syncContractAccessPassForProfile,
...
export async function POST() {
  try {
    ...
    let pass = await getActiveAccessPassForProfile(profile.id);

    if (!pass && DOGECORD_ACCESS_PASS.contractAddress) {
      try {
        pass = await syncContractAccessPassForProfile(profile);
      } catch (error) {
        console.log("[SETUP_PASS_SYNC_MISS]", error);
        pass = null;
      }
    }
